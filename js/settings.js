...
document.getElementById('settingsSaveBtn').addEventListener('click', () => {
  safeSetItem(LS_PROVIDER, providerSelect.value);
  safeSetItem(LS_GEMINI_KEY, document.getElementById('apiKeyInputGemini').value.trim());
  safeSetItem(LS_GEMINI_MODEL, document.getElementById('geminiModelSelect').value);
  safeSetItem(LS_GROQ_KEY, document.getElementById('apiKeyInputGroq').value.trim());
  safeSetItem(LS_GROQ_MODEL, document.getElementById('groqModelSelect').value);
...
function refreshBackupNote() {
  ...
  refreshStorageUsageNote();
}

// Rough storage-usage bar against a conservative 5MB baseline (the
// number most browsers guarantee at minimum; several allow more, but
// this keeps the bar from understating risk on a stricter browser).
// Not exact — see estimateStorageUsageBytes()'s own comment — but
// gives the user an early "you're getting full" signal well before
// they actually hit a failed save.
const STORAGE_ROUGH_LIMIT_BYTES = 5 * 1024 * 1024;

function refreshStorageUsageNote() {
  const noteEl = document.getElementById('storageUsageNote');
  const fillEl = document.getElementById('storageUsageBarFill');
  if (!noteEl || !fillEl) return;

  const usedBytes = estimateStorageUsageBytes();
  const pct = Math.min(100, Math.round((usedBytes / STORAGE_ROUGH_LIMIT_BYTES) * 100));

  fillEl.style.width = `${pct}%`;
  fillEl.classList.toggle('warn', pct >= 80);

  if (pct >= 80) {
    noteEl.textContent = `Using about ${formatBytes(usedBytes)} — getting close to what most browsers allow (~5-10MB). Consider exporting a backup, then deleting old decks/notes/watch sessions.`;
    noteEl.style.color = 'var(--danger)';
  } else {
    noteEl.textContent = `Using about ${formatBytes(usedBytes)} of local storage (estimate).`;
...
function importBackupData(file) {
  ...
  reader.onload = () => {
    ...
    // Rough pre-flight check: if the backup itself is larger than
    // what's realistically available, warn up front rather than
    // discovering it key-by-key mid-import (which would leave the
    // dataset in a half-old/half-new state). This is an estimate, not
    // a guarantee — actual browser quotas vary — so it warns and lets
    // the user decide, rather than blocking the import outright.
    const incomingBytes = keysFound.reduce((sum, k) => sum + k.length + String(parsed.data[k]).length, 0) * 2;
    const currentBytes = estimateStorageUsageBytes();
    const roughLimitBytes = 5 * 1024 * 1024; // conservative 5MB baseline most browsers guarantee
    if (incomingBytes > roughLimitBytes) {
      const proceedAnyway = confirm(
        `This backup is ${formatBytes(incomingBytes)}, which is large enough that it may not all fit ` +
        `(browsers typically allow ~5-10MB total, and you're currently using about ${formatBytes(currentBytes)}). ` +
        `Import anyway? If it fails partway through, you'll see exactly which parts didn't restore.`
      );
      if (!proceedAnyway) {
        setStatus('Import cancelled.', false);
        return;
    ...
    keysFound.forEach(key => {
      const ok = safeSetItem(key, parsed.data[key]);
    ...
    } else if (failedKeys.length === keysFound.length) {
      // Nothing restored at all — most likely storage is completely full.
      setStatus(`Import failed — your browser's storage is full, so none of the ${keysFound.length} item(s) could be restored. Free up space (delete old decks/notes/watch sessions) and try again.`, true);
    } else {
      // Partial restore: some keys made it in before storage filled up.
      // Reload anyway so the successfully-restored parts take effect,
      // but be explicit that it's incomplete rather than saying
      // "complete" when it wasn't.
      setStatus(`Storage filled up partway through — restored ${keysFound.length - failedKeys.length} of ${keysFound.length} item(s). Free up space and re-import to get the rest. Reloading…`, true);
